...
test_create_group_stop_daemon_then_recreate()
{
  ...
  if [ "${scenario}" = 'no_stop' ]; then
    ...
    # remove and recreate group - currently fails with 2 keys due to bug- ISSUE 24 TODO
    group_remove "${primary_cluster}" "${pool}/${group}"
    group_create "${primary_cluster}" "${pool}/${group}"
    mirror_group_enable "${primary_cluster}" "${pool}/${group}"
...
test_odf_failover_failback()
{
  ...
  # promote secondary, demote original primary and request resync
  stop_mirrors "${secondary_cluster}"
  mirror_group_promote "${secondary_cluster}" "${pool}/${group0}" '--force'
  start_mirrors "${secondary_cluster}"
  ...
  local group_snap_id_c group_snap_id_d
  get_newest_group_snapshot_id "${secondary_cluster}" "${pool}"/"${group0}" group_snap_id_c
  get_newest_group_snapshot_id "${primary_cluster}" "${pool}"/"${group0}" group_snap_id_d
  test "${group_snap_id_a}" != "${group_snap_id_c}" || fail "new snap not taken by demote"
  test "${group_snap_id_a}" = "${group_snap_id_d}" || fail "new snap on original primary"
  ...
  if [ "${scenario}" = 'retry_promote' ]; then
    while true; do
      ...
      # TODO this variant eventually passes, but there are >30 group snapshots on the primary afterwards 
    done
  else
    # TODO Test fails on next step complaining about read-only filesystem
  ...
  # after the promote succeeds
  # check that group and images were deleted and recreated on secondary cluster
  # Write some data, take a regular mirror snapshot, wait for it to sync on secondary cluster and assert the local group and image IDs changed on site-b (edited) 
}
...
test_resync_marker()
{
  ...
  # try to wait here as a workaround - makes no difference TODO remove
  sleep 30

  # TODO next command fails
...
# TODO implement steps 9,10,11
  ...
  local group_id_after
  get_id_from_group_info ${primary_cluster} ${pool}/${group0} group_id_after
  echo_red "${group_id_before}:${group_id_after}"
  test "${group_id_before}" != "${group_id_after}" || fail "group was not recreated"
