  ...
  optional_yield y)
{
  ...
  for (uint16_t attempt = 1; /* empty */; ++attempt) {
	...
	next_marker = index_key;
      ...
      if (params.filter_trash && obj.name == RGW_TRASH_RESERVATION_PREFIX) {
          continue;
      }
                                                   ...
                                                   optional_yield y)
{
  ...
  while (truncated && count <= max) {
	...
	continue;
      ...
      if (params.filter_trash && params.prefix.length() == 0 && obj.name == ".trash" + params.delim) {
          continue;
      }
...
int RGWRados::Object::Delete::restore_obj(optional_yield y, const DoutPrefixProvider *dpp)
{
    ...
    r = restore_index_op.complete(dpp, obj_in_trash_dirent.ver.pool, obj_in_trash_dirent.ver.epoch,
