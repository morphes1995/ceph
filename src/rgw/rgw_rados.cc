...
RGWObjState::RGWObjState(const RGWObjState& rhs) : obj (rhs.obj) {
  ...
  inlined = rhs.inlined;
  head_exists = rhs.head_exists;
  if (rhs.head_obj_tag.length()) {
    head_obj_tag = rhs.head_obj_tag;
  }
  head_manifest = rhs.head_manifest;
  merge_obj_name = rhs.merge_obj_name;
  offset = rhs.offset;
}
                                           ...
                                           void *_index_op, optional_yield y)
{
  ...
  if (!index_op->is_prepared()) {
    auto before_put_op_bi_prepare = ceph_clock_now();
    ...
    auto after_put_op_bi_prepare = ceph_clock_now();
  ...
  auto& ioctx = ref.pool.ioctx();
  auto before_put_op_write_head = ceph_clock_now();
  tracepoint(rgw_rados, operate_enter, req_id.c_str());
  r = rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &op, null_yield);
  tracepoint(rgw_rados, operate_exit, req_id.c_str());
  auto after_put_op_write_head = ceph_clock_now();
  ...
  auto before_put_op_bi_complete = ceph_clock_now();
  auto after_put_op_bi_complete = ceph_clock_now();
  ...
  before_put_op_bi_complete = ceph_clock_now();
  ...
  after_put_op_bi_complete = ceph_clock_now();
                        ...
                        map<std::string, bufferlist>& attrs, optional_yield y)
{
    ...
    if (meta.ptag){
      ...
      append_rand_alpha(store->ctx(), ptag, ptag, 32);
    }
    index_op.set_op_tag(ptag);

    bufferlist tag_bl;
    tag_bl.append(ptag.c_str(), ptag.size() + 1);
    ldpp_dout(dpp, 10) << "setting object write_tag=" << ptag << dendl;
...
//    }
    ...
    auto before_complete_atomic = ceph_clock_now();
    ...
    /* update quota cache */
    if (meta.completeMultipart){
        store->quota_handler->update_stats(meta.owner, obj.bucket, 1, 0, 0); // always assume object didn't exists
    }
    else {
    ...
    store->dc->bucket_has_inlined_obj(obj.bucket, bucket_info.placement_rule);
...
void RGWObjStateAioManager::do_completion(bool is_from_head, const string &oid) {
  ...
  if (is_from_head) {
    ...
    ldpp_dout(dpp, 20) << "async op to get rgw object state from head obj:  "  << oid << " finished, time: " << ceph_clock_now()
                       << " ret: " << completion_from_head->get_return_value()  << " time taken: "<< head_op_end - head_op_start << dendl;
  }else {
    completion_from_bi_entry = pending_from_bi_entry;
    pending_from_bi_entry = NULL;
    auto bi_entry_op_end = ceph_clock_now();
    ldpp_dout(dpp, 20) << "async op to get rgw object state from bi entry:  "  << oid << " finished, time: " << ceph::real_clock::now()
                       << "ret: " << completion_from_bi_entry->get_return_value()
                       << "time taken: "<< bi_entry_op_end - bi_entry_op_start << dendl;
  }

  if (pending_from_head == NULL && pending_from_bi_entry == NULL){
    cond.notify_one();
    ldpp_dout(dpp, 20) << "all async op finished, notify time: " << ceph_clock_now() << dendl;
  }
}
...
int RGWConcurrentGetObjState::issue_op(uint64_t *psize, ceph::real_time *pmtime, uint64_t *epoch, map<string, bufferlist> *attrs,
                                       ...
                                       bool* inlined, bool* head_exists, bufferlist *head_obj_tag, uint64_t *head_rados_size, string *merge_obj_name, uint32_t *offset) {
  int r = 0;
  ...
  ldpp_dout(dpp, 20) << "all async ops to get rgw object state finished,time :" << ceph_clock_now() <<
                        "head oid:" << head_oid << " shard oid :" << shard_oid
  ...
  // we should always select the latest one, consider the following cases:
  ...
  if(head_ret_code == -ENOENT){
    if (!dirent.meta.inline_head){
      ...
      return -ENOENT;
    }else if (dirent.meta.inline_head && !dirent.exists) {
      // case 2: head rados object didn't exist, meanwhile, inlined bucket index entry don't exist
      if (inlined){
        *inlined = true;
      }
      return -ENOENT;
    } else {
      // case 3: head rados object didn't exist, meanwhile, inlined bucket index entry exists
      if (inlined){
        *inlined = true;
      }
      if (head_exists){
      ...
      return parse_bi_entry_as_result(dirent, psize, pmtime, epoch, attrs, first_chunk, merge_obj_name, offset, objv_tracker);
    }
  }

  if (head_ret_code >= 0){
    if (!dirent.meta.inline_head){
      // case 4:  head rados object exist, no inlined bucket index entry
      if (inlined){
        *inlined = false;
      }
      parse_head_as_result(psize, pmtime, epoch, attrs, first_chunk, objv_tracker);
    }else {
      // case 5: both head rados object and inlined bucket index entry exist
      ...
      if(dirent.meta.inline_index_epoch > epoch_in_head){
        ...
        if (head_obj_tag){
          *head_obj_tag = result_from_head.unfiltered_attrset[RGW_ATTR_ID_TAG];
        }

        if (head_rados_size){
          (*head_rados_size) = result_from_head.size;
        }

        if (dirent.exists){
          r = parse_bi_entry_as_result(dirent, psize, pmtime, epoch, attrs, first_chunk, merge_obj_name, offset, objv_tracker);
        }else {
          r = -ENOENT; // treat this non-existence inlined entry as deleter delete marker
        }
        if (attrs) {
          (*attrs)[RGW_ATTR_HEAD_MANIFEST] = result_from_head.unfiltered_attrset[RGW_ATTR_MANIFEST];
        }
      }else{
        if (inlined){
          *inlined = false;
...
int RGWConcurrentGetObjState::parse_bi_entry_as_result(rgw_bucket_dir_entry &dirent, uint64_t *psize, ceph::real_time *pmtime, uint64_t *epoch,
  ...
  if(merge_obj_name){
    *merge_obj_name = dirent.meta.merge_obj_name;
  }
  if(offset){
...
rgw::sal::RGWRadosStore* DCWorkQ::get_store(){
  return wk->get_store();
}
...
void *DCWorkQ::entry() {
  while (!wk->going_down()) {
    ...
    while (truncated) {
      ...
      if(r == -ECANCELED){
        ...
        if (rc < 0 ) {
          ldpp_dout(dpp, 1) << "WARNING DC WorkerQ[" << thr_name() << "] list bucket: " << shardItem.bucket.name
                            << " shard:" << shardItem.oid << " lease acquired by other thread, we handle next shard.  r: " << rc << dendl;
          break;
        }else {
          continue;
        }
      ...
      if (r < 0){
        ldpp_dout(dpp, 1) << "ERROR DC WorkerQ[" << thr_name() << "] list bucket: " << shardItem.bucket.name
...
struct clear_op_data {
    ...
    void handle_completion(completion_t cb) {
      int r = rados_aio_get_return_value(cb);
      if (r<0){
        ldpp_dout(dpp, 1) << "ERROR failed to clear inlined head data,  r: " << r << dendl;
      }
    }
};

static void clear_head_cb(completion_t cb, void *arg)
{
  clear_op_data *completion = (clear_op_data *)arg;
  completion->handle_completion(cb);
  delete completion;
}

int DCWorkQ::try_clear_stale_head(ShardItem &shardItem, rgw_bucket_dir_entry &dirent){
...
void DCWorkQ::batch_detach_and_merge(ShardItem &shardItem, rgw_bucket_dir &dir, const string &sc, list<rgw_bucket_dir_entry> &entries_to_merge){
  ...
  if(!entries_to_merge.empty()){
    ...
    if(async_clear){
      ...
      entry->dpp = dpp;
      AioCompletion *c = librados::Rados::aio_create_completion(entry, clear_head_cb);
      r = ioctx.aio_operate(oid, c, &op);
    }else{
...
int DCWorkQ::_merge_heads_payload(ShardItem &shardItem, rgw_bucket_dir &dir,
  ...
  do{
    ...
    if(r < 0 && r != -ENOENT){
      ldpp_dout(dpp, 1) << "ERROR DC WorkerQ[" << thr_name() << "] get merge obj state :" << merge_obj_name << " r: " << r << dendl;
      return r;
    }
    if (r == -ENOENT){
      size = 0;
...
rgw::sal::RGWRadosStore* DCWorker::get_store(){
  return dc->store;
}

void DCWorker::enqueue_bucket(librados::IoCtx& io_ctx, std::map<int, std::string>& oids, rgw_bucket &bucket, rgw_placement_rule &rule, std::string &rgw_instance){
  for (const auto &entry: oids){
    int tix = entry.first % wqs.size();
    ShardItem item;
    item.oid = entry.second;
    item.shard_id = entry.first;
    item.bucket = bucket;
    item.index_pool_io_ctx = io_ctx;
    item.placement_rule = rule;
    item.rgw_instance = rgw_instance;
...
void RGWRadosDetacher::bucket_has_inlined_obj(rgw_bucket& bucket, rgw_placement_rule &placement_rule){
  ...
  modified_buckets[bucket] = placement_rule;
  detach_thread->try_wakup();
}

void RGWRadosDetacher::swap_modified_buckets(map<rgw_bucket, rgw_placement_rule>& out) {
  std::unique_lock lock{mutex};
...
void RGWRadosDetacher::try_disable_object_inline(string &bucket_id){
  ...
  if(inlined_entry_num == 0){
    ...
    // update object inlined bucket entry
    ret = set_entry(bucket->get_key(), false);
    if (ret < 0) {
...
int RGWRadosDetacher::vacuum_bucket(string &bucket_id, utime_t &start){
  vector<std::string> fields;
  boost::split(fields, bucket_id, boost::is_any_of(":"));
  string bucket_tenant = fields[0];
  string bucket_name = fields[1];
  ...
  double threshold = cct->_conf->rgw_vacuum_threshold_pct;
  for(auto &sc: merge_objects_stale_frags.stats){
    ...
    for(auto &shard: sc.second){
      ...
      for(const auto &obj: shard.second){
        ...
        if (!merge_obj.writing && double(merge_obj.size_to_release) / double(merge_obj.size) >= threshold ){
          vacuum_object(bucket.get(), storage_class, shard_id, merge_obj_id, merge_obj);

          utime_t now = ceph_clock_now();
          if( (now.to_msec() - start.to_msec()) >= (cct->_conf->rgw_vacuum_process_period_sec * 1000) / 2){
            return -ECANCELED; // we processed to many merge object
          }
          if (going_down()){
            return -ECANCELED;
          }
...
// during the method execution, deletion related to src_merge_obj may occur simultaneously,
// so extra stale frags may be generated
void RGWRadosDetacher::vacuum_object(rgw::sal::RGWBucket *bucket, string &sc, uint16_t shard_id, uint32_t merge_obj_id, const rgw_merge_object_stat &src_merge_obj){
  ...
  ldpp_dout(dpp, 10) << __func__ << " vacuum merge obj:" << src_merge_obj_name << "sc: " << sc
  ...
  if(src_merge_obj.size_to_release == src_merge_obj.size){
    remove_fully_stale_obj(bucket, sc, shard_id, src_merge_obj_name);
    return;
  }

  // 1. read merge obj stale frags from all shards;
  map<uint32_t, rgw_merge_obj_stale_frag> stale_frags;
  int r = store->getRados()->list_stale_frags(dpp, bucket->get_info(), sc, src_merge_obj_name, stale_frags);
  if (r < 0){
    return;
  }

  uint32_t total_frags_size = 0;
  ...
  ObjectReadOperation op;
  std::map<std::string,bufferlist> omap_offsets;
  op.omap_get_vals2("", LONG_MAX, &omap_offsets, nullptr, &r);
  if (r < 0) {
    ldpp_dout(dpp, 1) << "failed to create read op of : " << src_merge_obj_name << " r:" << r << dendl;
    return ;
  }
  r = rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &op, nullptr, null_yield);
  if (r < 0) {
    ldpp_dout(dpp, 1) << "failed to read omap offsets of merge obj: " << src_merge_obj_name << " r:" << r << dendl;
    return ;
  }
  std::map<uint32_t, rgw_object_offset> offsets;
  for(auto &item : omap_offsets){
    rgw_object_offsets_info info;
    try {
      decode(info, item.second);
    } catch (buffer::error& err) {
      ldpp_dout(dpp, 0) << "ERROR: failed to decode rgw_object_offsets_info, err: " << err.what() << dendl;
      return;
    }

    for(auto &off: info.offsets){
      offsets[off.offset] = off;
    }
  }
  ...
  for(auto &item : offsets){
    ...
    if(prev_off != -1){
      ceph_assert((uint32_t)prev_off < item.second.offset);
    }
  ...
  // 4. create dest merge object
  string dest_merge_obj_name = "evoc.merged.big.obj_" + to_string(shard_id)
                               + "_" + to_string(merge_obj_id) + "_" +to_string(src_merge_obj.version+1);
  rgw_raw_obj dest_raw_merge_obj;
  rgw_obj dest_rgw_merge_obj(bucket->get_info().bucket, dest_merge_obj_name);
  store->get_raw_obj(rule, dest_rgw_merge_obj, &dest_raw_merge_obj);
  rgw_rados_ref dest_ref;
  r = store->getRados()->get_raw_obj_ref(dpp, dest_raw_merge_obj, &dest_ref);
  if (r < 0) {
    ldpp_dout(dpp, 1) << "failed to get dest merge obj ref " << dest_merge_obj_name << dendl;
    return ;
  }
  ObjectWriteOperation create_op;
  create_op.create(true);
  std::map<std::string,bufferlist> dest_omap_offsets = {{"offsets_0", new_offsets_bl}};
  create_op.omap_set(dest_omap_offsets);

  r = rgw_rados_operate(dpp, dest_ref.pool.ioctx(), dest_ref.obj.oid, &create_op, null_yield);
  if (r < 0) {
    ldpp_dout(dpp, 1) << "failed to create dest merge obj: " << "dest_merge_obj_name" << " r:" << r << dendl;
    return ;
  }
  ...
  while(iter != offsets.end()){
    ...
    int off = -1;
    int len = 0;
    while(iter != offsets.end() && stale_frags.find(iter->first) == stale_frags.end()){
      if (off == -1){
          off = iter->second.offset;
      }
    ...
    if(len > 0){
      // 5.1 read contiguous bytes from src
      ObjectReadOperation rop;
      rop.read(off, len, &outbl, &r);
      if (r < 0) {
        ldpp_dout(dpp, 1) << "failed to create read op for : " << src_merge_obj_name
                          << " offset:" << off <<  " r:" << r << dendl;
        return ;
      }
      r = rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &rop, nullptr, null_yield);
      if (r < 0) {
        ldpp_dout(dpp, 1) << "failed to read data from : " << src_merge_obj_name
                          << " offset:" << off <<  " r:" << r << dendl;
        return ;
      }
  ...
  r = store->getRados()->finish_vacuum(dpp, bucket->get_info(), sc, shard_id, src_merge_obj_name, dest_merge_obj_name, dest_merge_obj, new_offsets_info);
  if (r < 0) {
    ldpp_dout(dpp, 1) << "vacuum finish failed : "<<  " r:" << r << dendl;
    ObjectWriteOperation del_op;
    list<string> prefixes;
    cls_rgw_remove_obj(del_op, prefixes);
    r = rgw_rados_operate(dpp, dest_ref.pool.ioctx(), dest_ref.obj.oid, &del_op, null_yield);
    if (r < 0 && r != -ENOENT) {
      ldpp_dout(dpp, 1) << "ERROR failed to delete src merge obj :" << dest_merge_obj_name << " r: " << r << dendl;
    }
    return ;
  }

  // 9. remove src merge object
  ObjectWriteOperation del_op;
  list<string> prefixes;
  cls_rgw_remove_obj(del_op, prefixes);
  auto& ioctx = ref.pool.ioctx();
  r = rgw_rados_operate(dpp, ioctx, ref.obj.oid, &del_op, null_yield);
  if (r < 0 && r != -ENOENT) {
...
void RGWRadosDetacher::remove_fully_stale_obj(rgw::sal::RGWBucket *bucket, string &sc, uint16_t shard_id, string &src_merge_obj_name){
  rgw_merge_object_stat dummy_obj;
  rgw_object_offsets_info dummy_offsets_info;
  string dummy_dest_merge_obj_name = "";
  // update shard head stats
  // remove stale fragments entries
  int r = store->getRados()->finish_vacuum(dpp, bucket->get_info(), sc, shard_id, src_merge_obj_name, dummy_dest_merge_obj_name, dummy_obj, dummy_offsets_info);
  if (r < 0) {
...
{
  std::optional<RGWObjManifest> manifest = state->manifest;
  if (state->inlined && state->head_exists) {
    manifest = state->head_manifest;
  }
...
int RGWRados::Object::Delete::copy_head_and_bi_to_trash_bin(optional_yield y, const DoutPrefixProvider *dpp){
    ...
    if (state->data.length() != state->manifest->get_head_size()){
        ldpp_dout(dpp, 0) << "ERROR: when copy_head_and_bi_to_trash_bin , head obj len mismatch !, stat->data.len: "
                          << state->data.length() << " state->manifest->get_head_size(): " << state->manifest->get_head_size() << dendl;
        return -EIO;
    }
...
int RGWRados::Object::Delete::rename_bi_entry_to_trash_bin(const DoutPrefixProvider *dpp){
  ...
  RGWObjState *state;
  int r = target->get_state(dpp, &state, false, null_yield);
  if (r < 0)
    return r;
  if (!state->exists) {
    ldpp_dout(dpp, 0) << "ERROR: head did not exist when move to obj (" << state->obj.key.name << ") to trash bin, r:" << r << dendl;
  ...
  std::stringstream ss;
  ss << state->mtime;
  string dest_name = RGW_TRASH_RESERVATION_PREFIX + obj.key.name + ".CREATED_AT_" + ss.str(); // obj name with create time
  r = store->bi_rename(dpp, target->get_bucket_info().bucket, obj, obj.key.name, dest_name, state->obj_tag, true);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "WARNING: failed to rename bi entry (" << obj.key.name << ") in trash bin. r:" << r << dendl;
...
int RGWRados::Object::Delete::rename_bi_entry_from_trash_bin(const DoutPrefixProvider *dpp, rgw_obj& trash_obj, bufferlist &trash_obj_tag, rgw_obj& origin_obj){
  RGWRados *store = target->get_store();

  // read origin object bucket index entry, check non-existence
  rgw_bucket_dir_entry dirent;
  int r = store->bi_get_plain(dpp, target->get_bucket_info(), origin_obj, &dirent);
  if (r < 0 && r != -ENOENT) {
    ldpp_dout(dpp, 0) << "ERROR: read origin object(" << origin_obj.key.name << ") bi entry error, r:" << r << dendl;
    return r;
  }
  if (dirent.exists){
    ldpp_dout(dpp, 1) << "WARNING: restore trash obj should not overwrite existed origin object!, "
...
int RGWRados::Object::Delete::delete_tiny_object(const DoutPrefixProvider *dpp, RGWObjState *state) {
  if (!state->exists) {
    target->invalidate_state();
    return -ENOENT;
  }
  ...
  if (!real_clock::is_zero(params.unmod_since)) {
    ...
    index_op.need_check_mtime( params.unmod_since, params.high_precision_time, CLS_RGW_CHECK_TIME_MTIME_LE);
  }
  ...
  if (params.bucket_trash_bin_enabled) {
    /**
     * the delete tiny object move to trash , only when:
    ...
    if (!params.obj_in_bucket_trash_bin && !params.del_obj_bypass_trash_bin){
      return rename_bi_entry_to_trash_bin(dpp);// copy head and bi entry to trash bin on delete
    }
  }
  ...
  index_op.set_op_tag(state->obj_tag.c_str());
  ...
  target->get_store()->dc->bucket_has_inlined_obj(obj.bucket, bucket_info.placement_rule);
...
int RGWRados::Object::Delete::delete_obj(optional_yield y, const DoutPrefixProvider *dpp)
{
  ...
  if (state->inlined) {
    return delete_tiny_object(dpp, state);
  }
...
int RGWRados::Object::Delete::restore_obj(optional_yield y, const DoutPrefixProvider *dpp)
{
    ...
    // timestamp suffix len is 43, example sizeof(".CREATED_AT_2025-11-11T06:59:35.403942+0000") -1
    ...
    if(state->inlined){
      return rename_bi_entry_from_trash_bin(dpp, obj_in_trash, state->obj_tag, origin_obj);
    }
    ...
    // 3. read trash obj bucket index entry
    obj_in_trash.index_hash_source =  origin_obj.key.name; // trash obj on the shard where origin obj located
    ...
    // 4. read origin object bucket index entry, check non-existence
    rgw_bucket_dir_entry dirent;
    ...
    // 5. restore object (head and bi entry) from trash (may race with concurrent put op)
    ...
    r = restore_index_op.complete(dpp, ioctx.get_id(), ioctx.get_last_version(),
                                  ...
                                  &obj_in_trash_dirent.meta.user_data, obj_in_trash_dirent.meta.appendable, false, true/* bilog do not generate here */);
    if (r < 0){
    ...
    obj_in_trash_dirent.meta.mtime = real_clock::now();;
    obj_in_trash_dirent.ver.pool = obj_in_trash_ref.pool.ioctx().get_id();
    obj_in_trash_dirent.ver.epoch = obj_in_trash_ref.pool.ioctx().get_last_version();

    encode(obj_in_trash_dirent, entry_to_del.data);
                                 ...
                                 RGWObjState **state, bool follow_olh, optional_yield y, bool assume_noent)
{
  ...
  *state = s;
  ...
  if (!assume_noent) {
    if (obj.key.get_ns() != "multipart"){
      ...
      r = concurrentGetState.issue_op(&s->size, &s->mtime, &s->epoch, &s->attrset, (s->prefetch_data ? &s->data : NULL), NULL,
                                      &s->inlined, &s->head_exists, &s->head_obj_tag, &s->head_rados_size, &s->merge_obj_name, &s->offset);
      auto after_time = ceph_clock_now();
  ...
  if (s->inlined && s->head_exists){
    bufferlist manifest_bl = s->attrset[RGW_ATTR_HEAD_MANIFEST];
    if (manifest_bl.length()) {
      auto miter = manifest_bl.cbegin();
      try {
        s->head_manifest.emplace();
        decode(*s->head_manifest, miter);
        s->head_manifest->set_head(bucket_info.placement_rule, obj, s->head_rados_size);
      } catch (buffer::error& err) {
        ldpp_dout(dpp, 0) << "ERROR: couldn't decode head manifest" << dendl;
        return -EIO;
      }
      ldpp_dout(dpp, 10) << "head_manifest: total_size = " << s->head_manifest->get_obj_size() << dendl;
    }
  }
                                                  ...
                                                  bool modify_tail, optional_yield y)
{
  ...
  bool need_guard = ((state->manifest) || (state->obj_tag.length() != 0) ||
                     (state->head_manifest) || (state->head_obj_tag.length() != 0) ||
  ...
  if (need_guard) {
    ...
    if (if_nomatch == NULL || strcmp(if_nomatch, "*") != 0) {
      ...
      if (!state->inlined){
        op.cmpxattr(RGW_ATTR_ID_TAG, LIBRADOS_CMPXATTR_OP_EQ, state->obj_tag);
      }else if (state->inlined && state->head_exists){
        op.cmpxattr(RGW_ATTR_ID_TAG, LIBRADOS_CMPXATTR_OP_EQ, state->head_obj_tag);
      }
    }
...
int RGWRados::Bucket::UpdateIndex::complete(const DoutPrefixProvider *dpp, int64_t poolid, uint64_t epoch,
                                            ...
                                            bool appendable, bool update_quota_stats, bool avoid_log_op)
{
  if (blind) {
    return 0;
  }
  ...
  ret = store->cls_obj_complete_add(*bs, obj, optag, poolid, epoch, ent, category, remove_objs, bilog_flags, zones_trace, update_quota_stats, avoid_log_op);
...
int RGWRados::Bucket::UpdateIndexAtomic::complete_atomic_add(const DoutPrefixProvider *dpp, uint64_t size,
                                 ...
                                 RGWObjState *astate, void *arg)
{
  ...
  if (astate && !astate->merge_obj_name.empty()) {
    ...
    ldpp_dout(dpp, 20) << "rados->get_obj_iterate_cb oid=" << merge_obj.oid << " obj-ofs=" << obj_ofs << " len=" << len << " chunk_len=" << chunk_len << dendl;
    op.read(astate->offset + obj_ofs, chunk_len, nullptr, nullptr);

    const uint64_t cost = chunk_len;
                          ...
                          iterate_obj_cb cb, void *arg, optional_yield y)
{
  ...
  if(!astate->merge_obj_name.empty()){
    rgw_obj merge_obj(bucket_info.bucket, astate->merge_obj_name);
    rgw_placement_rule rule = bucket_info.placement_rule;
    rule.storage_class = astate->manifest->get_tail_placement().placement_rule.storage_class;
    obj_to_raw(rule, merge_obj, &raw_merge_obj);
  }
                           ...
                           RGWObjVersionTracker *objv_tracker, optional_yield y)
{
  ...
  auto before_get_obj_stat = ceph_clock_now();
  bufferlist outbl;
  r = rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &op, &outbl, y);
  auto after_get_obj_stat = ceph_clock_now();
                                ...
                                map<uint32_t, rgw_merge_obj_stale_frag> &result)
{
  ...
  r = CLSRGWIssueListStaleFrags(index_pool.ioctx(), oids, list_result, sc, merge_obj_name, cct->_conf->rgw_bucket_index_max_aio)();
  if (r < 0) {
...
int RGWRados::bi_rename(const DoutPrefixProvider *dpp, rgw_bucket& bucket, rgw_obj& obj, string& src_name, string& dest_name,
                        bufferlist &obj_tag, bool to_trash)
{
  BucketShard bs(this);
  ...
  auto& ref = bs.bucket_obj.get_ref();
  ret = cls_rgw_bi_rename(ref.pool.ioctx(), ref.obj.oid, src_name, dest_name, obj_tag, to_trash);
  if (ret < 0)
    return ret;
...
int RGWRados::bi_ent_remove(const DoutPrefixProvider *dpp, rgw_bucket& bucket, rgw_obj& obj, rgw_cls_bi_entry& entry, uint16_t bilog_flag, bool log_op, rgw_zone_set *zones_trace)
{
    BucketShard bs(this);
    int ret = bs.init(bucket, obj, nullptr /* no RGWBucketInfo */, dpp);
    if (ret < 0) {
    ...
    auto& ref = bs.bucket_obj.get_ref();
    ret = cls_rgw_bi_ent_remove(ref.pool.ioctx(), ref.obj.oid, entry, bilog_flag, log_op, zones_trace);
    if (ret < 0)
        return ret;
...
int RGWRados::cls_obj_complete_op(BucketShard& bs, const rgw_obj& obj, RGWModifyOp op, string& tag,
                  ...
                  rgw_zone_set *_zones_trace, bool update_quota_stats, bool avoid_log_op)
{
  ObjectWriteOperation o;
  o.assert_exists(); // bucket index shard must exist
  ...
  cls_rgw_bucket_complete_op(o, op, tag, ver, key, dir_meta, remove_objs,
                             log_op, bilog_flags, &zones_trace, update_quota_stats);
...
int RGWRados::cls_obj_complete_add(BucketShard& bs, const rgw_obj& obj, string& tag,
                                   int64_t pool, uint64_t epoch,
                                   rgw_bucket_dir_entry& ent, RGWObjCategory category,
                                   list<rgw_obj_index_key> *remove_objs, uint16_t bilog_flags, rgw_zone_set *zones_trace, bool update_quota_stats, bool avoid_log_op)
{
  return cls_obj_complete_op(bs, obj, CLS_RGW_OP_ADD, tag, pool, epoch, ent, category, remove_objs, bilog_flags, zones_trace, update_quota_stats, avoid_log_op);
}
