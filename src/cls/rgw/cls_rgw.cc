...
#define BI_BUCKET_STALE_FRAG_INDEX    5
...
static std::string bucket_index_prefixes[] = { "", /* special handling for the objs list index */
                 ...
                 "3001_", /* merge object stale fragment index */
...
{
  key->name = index_key.substr(bucket_index_prefixes[BI_BUCKET_STALE_FRAG_INDEX].size() + 1);
  key->instance = "";
}
...
int rgw_bucket_set_merge_obj_stats(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "entered %s()\n", __func__);
  rgw_cls_bucket_set_merge_obj_stats_op op;
  auto iter = in->cbegin();
  try {
    decode(op, iter);
  } catch (ceph::buffer::error& err) {
...
static int do_rgw_bucket_complete_atomic_op2(cls_method_context_t hctx, rgw_bucket_dir_header &header, rgw_cls_obj_complete_op &op, bufferlist &queue_data){
  rgw_bucket_dir_entry entry;
  std::string idx;
  // controls whether remove_objs deletions are logged
  const bool default_log_op = op.log_op && !header.syncstopped;
...
int rgw_bucket_complete_atomic_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  int rc = read_bucket_header(hctx, &header);
  CLS_LOG(10, "INFO: %s read shard header time taken %ld ns", __func__, ceph_clock_now().to_nsec()- before_read_header);
  if (rc < 0) {
  ...
  CLS_LOG(10, "INFO: rgw_bucket_complete_atomic_op(): write %d bytes to shard obj data \n", queue_data.length());
  //write data size and data at tail offset
  ret = cls_cxx_write2(hctx, header.queue_tail, queue_data.length(), &queue_data, CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL);
  if (ret < 0) {
    return ret;
  }
...
int rgw_bucket_complete_atomic_op_batch2(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "entered %s", __func__);
  // decode request
  rgw_cls_obj_complete_op_batch op;
  ...
  bufferlist queue_data;
  for (auto &op: op.ops){
    rc = do_rgw_bucket_complete_atomic_op2(hctx, header, op, queue_data);
    if(rc < 0){
      return rc;
    }
  }
...
static int rgw_inlined_bi_rename_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if(entry.tag != op.obj_tag.c_str()){
    CLS_LOG(10, "WARNING: %s:  obj tag mismatch, op cancelled, key: %s", __func__, op.src_name.c_str());
    return -ECANCELED;
  }
  if(!op.to_trash){
                            ...
                            bool *pmore)
{
  ...
  if (max > 0) {
    ret = cls_cxx_map_get_vals(hctx, start_after_key, string(), max,
                               &keys, pmore);
    CLS_LOG(20, "%s: start_after_key=\"%s\", first_inlined_idx=\"%s\", keys.size()=%d",
            __func__, escape_str(start_after_key).c_str(),
...
static int list_stale_frag_entries(cls_method_context_t hctx,
                                      const string& name,
                                      const string& marker,
                                      uint32_t max,
                                      list<rgw_cls_bi_entry> *entries,
                                      bool *pmore)
{
  string first_stale_frag_idx;
  first_stale_frag_idx = BI_PREFIX_CHAR;
  first_stale_frag_idx.append(bucket_index_prefixes[BI_BUCKET_STALE_FRAG_INDEX]);
  first_stale_frag_idx.append(name);

  string start_after_key;

  if (!name.empty()) {
    start_after_key = first_stale_frag_idx;
  } else {
    start_after_key = BI_PREFIX_CHAR;
    start_after_key.append(bucket_index_prefixes[BI_BUCKET_STALE_FRAG_INDEX]);
  }
  string filter = start_after_key;
  if (bi_entry_gt(marker, start_after_key)) {
    start_after_key = marker;
  ...
  if (max > 0) {
    ret = cls_cxx_map_get_vals(hctx, start_after_key, string(), max,
                               &keys, pmore);
    CLS_LOG(20, "%s: start_after_key=\"%s\", first_stale_frag_idx=\"%s\", keys.size()=%d",
            __func__, escape_str(start_after_key).c_str(),
            escape_str(first_stale_frag_idx).c_str(), (int)keys.size());
    if (ret < 0) {
      return ret;
    }
  }
  ...
  for (auto iter = keys.begin(); iter != keys.end(); ++iter) {
    rgw_cls_bi_entry entry;
    entry.type = BIIndexType::StaleFrag;
    entry.idx = iter->first;
    entry.data = iter->second;
    ...
    cls_rgw_obj_key key;
    decode_stale_frag_key(entry.idx, &key);
			  ...
			  bufferlist *out)
{
  ...
  if (!more) {
    ...
    count += ret;
    CLS_LOG(20, "%s: found %d inlined index entries, count=%u", __func__, ret, count);
  }

  if (!more) {
    ret = list_stale_frag_entries(hctx, op.name_filter, op.marker, max - count, &op_ret.entries, &more);
    if (ret < 0) {
      CLS_LOG(0, "ERROR: %s: list_stale_frag_entries returned ret=%d", __func__, ret);
      return ret;
    }

    count += ret;
    CLS_LOG(20, "%s: found %d stale frag entries, count=%u", __func__, ret, count);
  }
...
static int rgw_cls_inline_set_entry_vacuuming(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if(op.rgw_vacuum_process_period_sec == 0){
    entry.vacuuming = false; // unset vacuuming
  }else{
    uint64_t  now = ceph_clock_now().to_msec();
    if (entry.vacuuming && (now - entry.vacuuming_start_time) <= op.rgw_vacuum_process_period_sec*1000){
...
static int rgw_cls_inline_is_entry_vacuuming(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "entered %s()\n", __func__);
  auto in_iter = in->cbegin();
                                        ...
                                        bufferlist *out)
{
  ...
  if (header.resharding()) {
    CLS_LOG(1, "WARNING: %s: resahrding, op canceled\n", __func__);
    ret.ret_code = -ECANCELED;
    encode(ret, *out);
    return 0;
  }
...
CLS_INIT(rgw)
{
  ...
  cls_method_handle_t h_rgw_bucket_set_merge_obj_stats;
  ...
  cls_method_handle_t h_rgw_bucket_complete_atomic_op_batch2;
  ...
  cls_method_handle_t h_rgw_inline_is_entry_vacuuming;
  ...
  cls_register_cxx_method(h_class, RGW_BUCKET_SET_MERGE_OBJ_STATS, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_set_merge_obj_stats, &h_rgw_bucket_set_merge_obj_stats);
  ...
  cls_register_cxx_method(h_class, RGW_BUCKET_COMPLETE_ATOMIC_OP_BATCH_NO_READ_ENTRY, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_complete_atomic_op_batch2, &h_rgw_bucket_complete_atomic_op_batch2);
  ...
  cls_register_cxx_method(h_class, RGW_INLINE_IS_ENTRY_VACUUMING, CLS_METHOD_RD, rgw_cls_inline_is_entry_vacuuming, &h_rgw_inline_is_entry_vacuuming);
